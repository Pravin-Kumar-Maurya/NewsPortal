...
def index(request):
    ...
    d=ncategory.objects.all().order_by("-id")[0:8]
    e = igallery.objects.all().order_by('-id')[0:8]
    ct = city.objects.all().order_by("-id")[0:8]
    x=mynews.objects.all().order_by("-id")[0:6]
    y = videonews.objects.all().order_by("-id")[0:4]
    mydict={"res":data,"data":d,"ndata":x,"vdata":y,"city":ct,"img":e}
...
def news(request):
    ...
    if ctid is not None:
        z=mynews.objects.all().filter(ncity=ctid).order_by("-id")
    elif catid is not None:
        z=mynews.objects.all().filter(category=catid).order_by("-id")
...
def viewnews(request):
    ...
    if a is not None:
        ...
        x = mynews.objects.all().filter(ncity=a).order_by('-id')
    mydict={"ndata":x}
...
def vdetail(request):
    ...
    md={"vdata":x}
    return render(request,'user/vdetail.html',context=md)
